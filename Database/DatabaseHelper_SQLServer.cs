...
{
    public class DatabaseHelper_SQLServer
    {
        ...
        public List<Appointment> GetAppointmentsByUserId(string userId)
        {
            ...
            using var conn = new SqlConnection(ConnectionString);
            conn.Open();
            var cmd = new SqlCommand("SELECT * FROM Appointments WHERE UserId = @UserId", conn);
        ...
        public void DeleteAppointment(string appointmentId)
        {
            using var conn = new SqlConnection(ConnectionString);
            conn.Open();
            var deleteReminders = new SqlCommand("DELETE FROM Reminders WHERE AppointmentId = @Id", conn);
        ...
        public void AddParticipantToGroup(string meetingId, string userId)
        {
            ...
            conn.Open();
            var cmd = new SqlCommand(@"
                IF NOT EXISTS (SELECT 1 FROM GroupMeetingParticipants WHERE MeetingId = @MeetingId AND UserId = @UserId)
                BEGIN
